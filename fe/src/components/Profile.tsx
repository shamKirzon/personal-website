...
const Profile = () => {
  ...
  return (
    <>
      ...
      >
        <div
          className="flex   flex-col items-center w-full max-w-md md:flex-row md:items-start
        ...
        >
          ...
          <div className="flex flex-col  items-center md:items-start">
            ...
            <p className=" text-zinc-500 text-center text-lg leading-6 mt-2 md:text-start">
              Software developer specializing in full-stack development and
              agentic AI application architecture.
            </p>

            <div className=" hidden  md:flex w-full md:mt-7 md:gap-x-5 lg:mt-6">
              {profileButtons.map((button, index) => (
                <button
                  key={index}
                  className={`flex items-center justify-center gap-x-1 bg-zinc-900 px-2 py-2 border-2 rounded-sm ${
                ...
                >
                  {button.icon}
                  <span className="text-md text-white">{button.label}</span>
                </button>
              ))}
            </div>
          </div>
